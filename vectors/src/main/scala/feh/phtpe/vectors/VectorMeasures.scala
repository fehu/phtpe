...
trait VectorMeasureOpsImplicits extends VectorOpsImplicits{
  ...
  {
    def vector[Tpe <: PhysType: PhysTypeDecomposition] = PhysTyped[V, Tpe](t)
  }
  ...
  implicit class VectorMeasureOps[V <: AbstractVector: VectorTypeEvidence, Tpe <: PhysType](v: Measure.Vector[V, Tpe]){
    ...
    def *(n: V#Num)(implicit ev: PhysTypeDecomposition[Tpe]): V|Tpe = v.value * n
    def *[Tpe2 <: PhysType](n: V#Num|Tpe2)(implicit ev: PhysTypeDecomposition[Tpe**Tpe2]): V|(Tpe**Tpe2) = v.value * n.value

    /** by-elem */
    def **[Tpe2 <: PhysType](v2: AbstractVector{ type Dim = V#Dim; type Num = V#Num }|Tpe2)
                            (implicit ev: PhysTypeDecomposition[Tpe**Tpe2]): V|(Tpe**Tpe2) = v.value ** v2.value

    /** dot product */
    def `.`[Tpe2 <: PhysType](v2: AbstractVector{ type Dim = V#Dim; type Num = V#Num }|Tpe2)
                             (implicit ev: PhysTypeDecomposition[Tpe**Tpe2]): V#Num|(Tpe**Tpe2) = (v.value `.` v2.value)
    def dot[Tpe2 <: PhysType](v2: AbstractVector{ type Dim = V#Dim; type Num = V#Num }|Tpe2)
                             (implicit ev: PhysTypeDecomposition[Tpe**Tpe2]): V#Num|(Tpe**Tpe2) = `.`(v2)
  }
...
//                                                                                       numV: Numeric[V],
                                                                                       d: PhysTypeDecomposition[Tpe])
  {
    def abs: V#Num|Tpe = v.value.abs
    def normalize: V|Tpe  = v.value.normalize
    def /(n: V#Num): V|Tpe = v.value / n
    def /[Tpe2 <: PhysType](v2: V#Num|Tpe2)(implicit ev2: PhysTypeDecomposition[Tpe/Tpe2]): V|(Tpe/Tpe2) = v.value / v2.value
  }

  implicit class Vector2DMeasureOps[V <: Vector2D: VectorTypeEvidence, Tpe <: PhysType: PhysTypeDecomposition](v: Measure.Vector[V, Tpe])
  {
  ...
  implicit class Vector3DMeasureOps[V <: Vector3D: VectorTypeEvidence, Tpe <: PhysType: PhysTypeDecomposition](v: Measure.Vector[V, Tpe])
  {
    private implicit def num = implicitly[VectorTypeEvidence[V]].num.asInstanceOf[Numeric[V#Num]]
    ...
    /** vector product */
    def X[Tpe2 <: PhysType](n: V|Tpe2)(implicit ev: PhysTypeDecomposition[Tpe**Tpe2]): V|(Tpe**Tpe2) = v.value X n.value
  }
}
...
trait VectorMeasuresTransformsImplicits extends VectorTransformImplicits{
  implicit class VectorMeasureTransform[V <: AbstractVector, Tpe <: PhysType](v: V|Tpe)(implicit ev: VectorTypeEvidence[V]){
    def to[N: Numeric](implicit transform: VectorNumTransform[V, N],
                       ...
                       d: PhysTypeDecomposition[Tpe]): AbstractVector{ type Dim = V#Dim; type Num = N }|Tpe = v.value.to[N]
  }
}